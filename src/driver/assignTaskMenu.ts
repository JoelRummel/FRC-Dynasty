...
const printTaskForce = (force: TaskForce, hasSecondary: boolean) => {
    ...
    console.log("MENTOR:      " + force.mentor ? force.mentor?.firstName : chalk.dim('<none>'))
    ...
    for (const student of force.students) {
        ...
        console.log(tab + buildStatBars({ building: student.skills.building }));
    }
    ...
    if (hasSecondary) {
        ...
        for (const student of force.softwareStudents!) {
            ...
            console.log(tab + buildStatBars({ programming: student.skills.programming }));
        }
    }
};
